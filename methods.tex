...
\comment{awk paragraph start} This is because archetypes store state as public or private member variables
...
performed prior to compilation and generated valid C++ \comment{awk}. Second, the
...
recognized and the directive \comment{is retained its output (awk)}. It is a
...
member variables that are saved and loaded from its instances are the most important \sout{.This is because it is} \comment{because} these variables \sout{that the} fully describe the state of an agent 
...
so that archetype developer need not write them. (Awk)}

The \cyclus preprocessor, called \cycpp, handles all of the cyclus code generation
and state accumulation. This tool looks only for directives that begin with \code{#pragma cyclus} so as to uniquely distinguish it from other preprocessors.
...
code generator. These hooks into \cycpp are exemplified by keys such as \code{'shape'}, 
...
downstream user interfaces\comment{should these keys be formatted as above, using the code command?}.  Even keys that might have a primary usage elsewhere, 
