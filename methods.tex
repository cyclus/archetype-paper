...
\code{#include} directives and implementing other \code{#} directives\comment{, such as \code{#pragma}s}. It is executed
...
recognized and the directive \comment{is retained its output (awk)}. \comment{Does this work? "Importantly, the \code{#pragma} directive is skipped by \code{cpp} if it is not recognized, so the directive retains its output."} It is a
