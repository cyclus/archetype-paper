...
directly available via the \code{annotations()} member function, which returns JSON 
object (equivalent to a Python dictionary with string keys). Combined with the 
autogenerated read-only keys from \cycpp, the metadata provides much needed 
reflection to the archetypes.  Unlike most C++ classes, archetypes have 
programatic access to their own class names, their parent classes, and the names and 
types of their state member variable at runtime. Therefore, the archtype may 
...
and other keys singal how archetypes and state variables should be treated in 
...
The var and note directives provide unambiquous locations for implementing and 
...
In a robust ecosystem of archetypes, it is nearly gaurenteed that different archetype
...
be concerned with accidiental name collision between their archetypes and archetypes
of other
...
and collection problems are simeltaneously solved through a hardy \emph{package system}.
...
to permenantly or temporarily alter the \cyclus search behavior.  Thus the path 
...
a file named \code{libmyagents.so} would recive the library name \code{myagents}
...
The path and library names together allow for complete disambiquation of 
...
for agent communication.  Furthermore, dynamic resource exhange is the keystone
...
guarenteing exchange feasibility. Moreover, all commodities are now traded through a
...
The archetype developer need not also portentially construct a custom exchange for 
each commodity an archetype trades. Furthermore, market removal does not 
impinge on exchange solver availablity or customization.  Many exchanges may 
be provided and have thier parameters tuned by the user.  The only resrtiction 
...
exhanges - arguably the most advanced task in \cyclus - have likely already 
