...
\Cyclus databases follow these fundemental abstractions:
\begin{itemize}
...
relatively simple C++ type \code{std::vector<int>} receieves two entries in the 
...
including nestings.
For example, the number of \cyclus types for \code{std::map<itnt, std::string>} is 
...
reason. \Cyclus uses the standard SHA1 \citeme algorithm to compute hash values
as 160-bit integer.  Thus for types with a fixed bit width less than 
...
\hline
\multicolumn{3}{|p{0.95\linewidth}|}{\textbf{DecomSchedule Table:}}\\
& & \\
\code{SimId} & \code{UUID} & Simulation identifier. \\
\code{AgentId} & \code{INT} & \\
\code{SchedTime} & \code{INT} & \\
\code{DecomTime} & \code{INT} & \\
\hline
\multicolumn{3}{|p{0.95\linewidth}|}{\textbf{BuildSchedule Table:}} \\
& & \\
\code{SimId} & \code{UUID} & Simulation identifier. \\
\code{ParentId} & \code{INT} & \\
\code{Prototype} & \code{VL_STRING} & \\
\code{SchedTime} & \code{INT} & \\
\code{BuildTime} & \code{INT} & \\
\hline
...
expressiveness given to the archtype developer by nativiley implemeneting complex types.
