...
may be thought of as a traditional parser that transforms tokens (line of the 
...
is a complex beast \comment{(too colloquial)} that allows for a number of programmer modifications prior 
...
simply to get the spelling correct. 
...
to be able to fill in the relevant value in the pass three code generation. Further 
...
process of building up archetype information for later use. Pass three of \cycpp where 
...
all of the member functions in Listing \ref{req-api}. However, in practice the body 
of each of these member functions follows its own pattern with respect to the 
state variables. This makes implementing pass three significantly easier.
...
Pass three necessarily must traverse all lines of the source code for the third time.
...
lines that an archetype developer must write per state variable. \sout{However} \comment{Moreover}, the 
...
there is less to learn about the \cyclus interface, and even for a developer who knows the \cyclus
...
by the need to be able to restart a simulation. \sout{On the other hand,} 
...
Simple types are represented by simple name: \code{float} becomes
...
\gls{SHA1} as a key\comment{(define SHA1?)}.  This data structure is a particular kind of associative array where the values are 
...
will grant one the value of the key respectively\comment{(something isn't adding up in the previous 2 sentences.  You can't get the key from knowing the value if the value isn't unique.  Typo somewhere?)}.  The \gls{HDF5} backend takes advantage 
...
\cyclus itself uses. Writing to such tables is \sout{considered} reserved to only
the kernel. 
...
described in Tables \ref{sv-anno} \& \ref{ag-anno}\comment{(these are both rendering as the same table number)}. 
...
\sout{However,} \gls{JSON} is a metadata representation that is meant to reside on disk, 
...
forth between \gls{JSON} types and C++ types as need. It provides a fully 
introspective 
...
implemented using robust schema validation with \acrlong{XML} and \acrlong{RNG} \comment{(add short acronym ref here as well, since it is first introduction of RelaxNG)}.
