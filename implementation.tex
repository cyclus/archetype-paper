...
and HDF5 \cite{folk2011overview}. These two represent relational and hierarchical 
databases respectively and have very different underlying design philosophies.
Future formats that could be supported are plain text comma-separated value files 
...
a distinction between fixed-length and variable-length (VL) versions. Thus even the 
...
will grant one the value of the key respectively.  The HDF5 backend takes advantage 
...
corresponding to their more powerful syntax. JSON is enough and just enough.

However, JSON is a metadata representation that is meant to reside on disk, 
...
\begin{enumerate}
    ...
    \item From C++ when \code{annotation()} is called, the JSON string is parsed 
...
implemented using XML's robust schema validation using the RelaxNG schema.
...
library to validate the provided input against the generated RelaxNG schema. If
